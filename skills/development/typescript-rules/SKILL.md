...
- Prefer `interface` for object shapes (e.g., React props)
- Prefer `interface` for public API types; or complex types that benefit from declaration merging
- Prepend `I` prefix to interfaces only when it adds clarity (e.g., `IUser` vs `UserProps`)
- Prepend `T` prefix to union/intersection types (e.g., `TSize` vs `Size`)
- Append `Type` postfix for internal types (e.g., `UserType` vs `User`); avoid postfixes for public API types
...
- Use `@shadcn/ui`, instead of raw HTML tags
